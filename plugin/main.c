...
/*
 * Repli d'identification quand ksceBtGetVidPid() echoue : toutes les DS4
 * s'annoncent « Wireless Controller ». Comparaison manuelle, str*() n'etant pas
 * disponible cote kernel.
 */
static int is_wireless_controller_name(const char *name)
{
	static const char ref[] = "Wireless Controller";
...
DECL_FUNC_HOOK(SceCtrl_sceCtrlGetBatteryInfo, int port, SceUInt8 *batt)
{
	...
	if (controllers[port].connected) {
		if (controllers[port].type == SCE_CTRL_TYPE_DS4) {
			SceUInt8 k_batt;
			ksceKernelMemcpyUserToKernel(&k_batt, (void *)batt, sizeof(k_batt));
			...
			ksceKernelMemcpyKernelToUser((void *)batt, &k_batt, sizeof(k_batt));
			return 0;
...
static void patch_motion_state_ds5(SceMotionState *motionState, struct ds5_input_report *ds5)
{
	...
	ksceKernelMemcpyUserToKernel(&k_data, (void *)u_data, sizeof(k_data));
	k_data.acceleration.x = ds5->accel_x;
	k_data.acceleration.y = ds5->accel_y;
	k_data.acceleration.z = ds5->accel_z;
	ksceKernelMemcpyKernelToUser((void *)u_data, &k_data, sizeof(k_data));
}
...
static int bt_cb_func(int notifyId, int notifyCount, int notifyArg, void *common)
{
	...
	while (1) {
		...
		case 0x01: { /* Inquiry result event */
			/*
			 * Meme defaut qu'au case 0x05, avec une consequence plus genante :
			 * l'amont ne recuperait meme pas le code de retour et testait un
			 * vid_pid non initialise. Si les residus de pile ressemblaient a une
			 * DS4, ksceBtStopInquiry() coupait la recherche Bluetooth en cours —
			 * y compris celle lancee depuis les parametres systeme, et pour
			 * n'importe quel appareil. D'ou une interface d'appairage qui ne
			 * trouve plus rien.
			 */
			unsigned short vid_pid[2] = { 0, 0 };
			unsigned int res;

			res = ksceBtGetVidPid(hid_event.mac0, hid_event.mac1, vid_pid);

			if (res == 0 && is_ds4(vid_pid)) {
				ksceBtStopInquiry();
				controller->mac0 = hid_event.mac0;
				controller->mac1 = hid_event.mac1;
			} else if (res != 0) {
				LOG("0x01 inquiry: GetVidPid=0x%08X sur %08X:%08X, ignore\n",
					res, hid_event.mac0, hid_event.mac1);
			}
			break;
		}
		...
		case 0x05: { /* Connection accepted event */
			/*
			 * vid_pid DOIT etre initialise : en cas d'echec de
			 * ksceBtGetVidPid(), l'amont laissait ce tableau rempli de residus
			 * de pile et les testait quand meme. Selon ce qui trainait, la
			 * manette etait prise pour une DS5 et recevait un report 0x31 de
			 * 73 octets au lieu du 0x11 de 13 octets : LED bleue, moteurs qui
			 * s'emballent, puis deconnexion. D'ou un comportement different a
			 * chaque tentative avec la meme manette.
			 */
			unsigned short vid_pid[2] = { 0, 0 };
			char name[0x79] = { 0 };
			...
			LOG("0x05 accepted: GetVidPid=0x%08X -> %04X:%04X | GetName=0x%08X -> \"%s\"\n",
				result1, vid_pid[0], vid_pid[1], result2, name);

			/* L'identification n'est fiable que si l'appel a reussi. */
			if (result1 == 0 && is_ds5(vid_pid)) {
			...
			} else if ((result1 == 0 && is_ds4(vid_pid)) ||
			           (result1 != 0 && result2 == 0 && is_wireless_controller_name(name))) {
				/*
				 * Second cas : l'identification par VID/PID a echoue, mais le
				 * peripherique s'annonce « Wireless Controller ». C'est le cas
				 * de la DS4 v2 de ce projet, qui ne repond pas aux requetes
				 * d'identification (meme trait qu'en USB avec le report 0x81).
				 * On la traite en DS4 plutot que de ne rien envoyer.
				 */
				...
				/*
				 * Amorcage de la boucle de lecture.
				 *
				 * enqueue_read_request() n'etait appele que depuis les cases
				 * 0x0A / 0x0B / 0x0C, c'est-a-dire en reponse a un rapport deja
				 * recu : chaque rapport demande le suivant. Personne n'envoyait
				 * la premiere requete. Cela fonctionnait quand meme pour les
				 * manettes prises en charge par le pilote natif, qui amorce la
				 * pompe de son cote ; pour une manette qu'il ignore — la DS4 v2
				 * de ce projet — aucune lecture n'etait jamais postee, donc
				 * aucun rapport d'entree ne remontait, et la manette se
				 * deconnectait au bout de quelques secondes faute d'activite.
				 *
				 * Mesure a l'appui : 1721 transferts de type 0 pour la v1 contre
				 * 0 pour la v2, alors que les deux recevaient les memes
				 * ecritures.
				 */
				enqueue_read_request(hid_event.mac0, hid_event.mac1,
					&hid_request, recv_buff, sizeof(recv_buff));

				LOG("     CONNECTED DS4[%08X %08X] TO PORT %i%s (lecture amorcee)\n",
					hid_event.mac0, hid_event.mac1, port,
					(result1 != 0) ? "  (identifiee par le nom)" : "");
	...
	/*
	 * Ecriture du journal apres chaque evenement traite. Sans cela, LOG() se
	 * contente de remplir un tampon de 16 Ko qui n'est vide qu'au dechargement
	 * du module, donc jamais en usage normal : les traces seraient perdues.
	 * Sur : bt_cb_func est un callback execute dans le contexte du thread BT,
	 * pas dans un hook.
	 */
	log_flush();
